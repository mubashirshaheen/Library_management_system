    ...
    public partial class purchase : Form
    {
        ...
        private void btnsave_Click(object sender, EventArgs e)
        {
            if (txtinv.Enabled == false)
            {
                ...
                if (r == DialogResult.Yes)
                {
                    ...
                    for (int i = 0; i < n; i++)
                    {
                        string q = "INSERT INTO tblpurchase (invoice,book_name,batch_no,author,quantity,tradeprice,rentprice) VALUES ('" + txtinv.Text + "','" + dgv1.Rows[i].Cells[0].Value + "','" + dgv1.Rows[i].Cells[1].Value + "','" + dgv1.Rows[i].Cells[2].Value + "','" + dgv1.Rows[i].Cells[3].Value + "','" + dgv1.Rows[i].Cells[4].Value + "','"+ dgv1.Rows[i].Cells[5].Value + "'); ";
                        fdata fd = new fdata();
                        fd.datasave(q);
                        int id = fd.checkdata(dgv1.Rows[i].Cells[0].Value.ToString());
                        if (id == 0)
                        {
                            q = "INSERT INTO tblstock (book_name,author,qty,rent_price) VALUES ('" + dgv1.Rows[i].Cells[0].Value + "','" + dgv1.Rows[i].Cells[2].Value + "','" + dgv1.Rows[i].Cells[3].Value + "','"+ dgv1.Rows[i].Cells[5].Value + "'); ";
                            fd.datasave(q);
                        }
                        else
                        {
                            ...
                            fd.datasave(q);
                            q = "UPDATE tblstock SET rent_price ='" + dgv1.Rows[i].Cells[5].Value + "' WHERE id='" + id + "';";
                            fd.datasave(q);
                        }
        ...
        private void dgv2_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            else
            {
                ...
                dgv1.Rows[n].Cells[5].Value = dgv2.SelectedRows[0].Cells[2].Value;               
                disabledgv();
                txtinv.Select();
            }
